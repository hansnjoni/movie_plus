...
QFrame#MovieCard { background-color: #3a0000; border-radius: 10px; border: 2px solid #ff0000; padding: 5px; margin: 2px; }
QPushButton { background-color: #111; color: #ff3333; border: 1px solid #aa0000; border-radius: 8px; padding: 6px; font-weight: bold; }
...
class MoviePlusPro(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Stark Cinema - Hard Cut v6.2"); self.resize(1550, 950); self.setStyleSheet(STYLESHEET)
        ...
        self.executor = ThreadPoolExecutor(max_workers=30); self.shown_ids = set(); self.init_ui()
    ...
    def init_ui(self):
        ...
        self.grid.setSpacing(10); self.scroll.setWidget(self.container); c_layout.addWidget(self.scroll); layout.addWidget(content); self.run_trending()
    ...
    def fetch_worker(self, url, t_id):
        try:
            ...
            while count <= 60 and page <= 40:
                ...
                if not raw: break
                self.signals.log_signal.emit(f"🔎 Scanning Page {page}...")
                for item in raw:
                    ...
                    # HARD FAILOVER JUMP
                    if self.recon_hard_jump(mid, mtype, mem):
                        self.shown_ids.add(mid); self.executor.submit(self.img_worker, item, count, mtype, t_id); count += 1
                        if count > 60: break
                page += 1
            self.signals.log_signal.emit(f"✅ Mission Complete: {count-1} Titles Ready.")
        except: pass

    def recon_hard_jump(self, mid, mtype, mem):
        if mid in mem and mem[mid]['status'] == "Available": return True
        # Jason will check Source A. If not ready in 1.0s, he drops it and tries Source B.
        for base in ["https://vidsrc.me/embed/", "https://vidsrc.to/embed/"]:
            try:
                url = f"{base}{mtype}?tmdb={mid}" if "me" in base else f"{base}{mtype}/{mid}"
                if requests.head(url, timeout=1.0).status_code == 200:
    ...
    def img_worker(self, item, rank, mtype, tid):
        ...
        try:
            ...
            self.signals.item_signal.emit(item, pix.scaled(170, 255, Qt.KeepAspectRatio, Qt.SmoothTransformation), rank, mtype, tid)
        except: pass
