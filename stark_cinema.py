...
SYSTEM_PROMPT = (
    ...
    "Hans is a master electrician, plumber, and rockhound from Kennedy, NY. "
    "You know about his 2004 Polaris Sportsman, his nephrite jade collection, "
    "and the 'Booster Station' logic used to fix audio stuttering. "
    "Today is April 2nd, his birthday. Be warm, witty, and efficient."
)
...
class StarkCinemaSingularity(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"Stark Cinema - Genesis V98.2")
        ...
        self.signals.log_signal.emit("⚡ [SYSTEM ONLINE]: All circuits hot. Happy Birthday, Hans.")

    def get_ai_response(self, text):
        try:
            response = self.chat_session.send_message(f"{SYSTEM_PROMPT}\n\nHans: {text}")
            return response.text
        except: return "Connection's flickering, Hans. Say again?"
    ...
    def speak(self, text):
        ...
        def run_speech():
            ...
            # SAPI Priority 2 for instant audio override
    ...
    def init_ui(self):
        self.setStyleSheet("""
            ...
            QPushButton { background-color: #1a0000; color: #FF0000; border: 1px solid #FF0000; padding: 12px; border-radius: 4px; }
            ...
            QLineEdit { background-color: #111; color: #00FF00; border: 1px solid #FF0000; padding: 8px; font-family: 'Consolas'; }
        """)
        ...
        self.search_bar = QLineEdit(); self.search_bar.setPlaceholderText("Awaiting Voice Command..."); c_layout.addWidget(self.search_bar)
    ...
    def add_item_to_ui(self, item, pix, rank, mtype, tid, s, e):
        if tid == self.task_counter: card = MovieCard(item, pix, mtype, self, s, e); self.grid.addWidget(card, (rank-1)//5, (rank-1)%5)
...
class MovieCard(QFrame):
    def __init__(self, item, pix, mtype, app, s, e):
        ...
        self.setStyleSheet("QFrame#MovieCard { border: 1px solid #FF0000; background-color: #000; }")
